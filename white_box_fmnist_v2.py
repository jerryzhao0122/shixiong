...
class WhiteBox(nn.Module):
    def __init__(self,hidden_layer_features_shape=None,param_gradients_shape=None):
        ...
        # encoder分类层
        self.encoder = FcnForEncoder(dim_in=64*8)
...
def load_target_model(model_path, device):
    # model = AlexNet(track=False).to(device)
...
def load_dataloader(file,id):
    ...
    label_dict = {}
    for idx,label in enumerate(untrained_dataset.train_labels):
    ...
    client0_dataset_train = MkFlDataset(trained_dataset,untrained_dataset,clients_samp_idx[id],train=True,idx2=idx2)
    client0_dataset_test = MkFlDataset(trained_dataset,untrained_dataset,clients_samp_idx[id],train=False,idx2=idx2)

    train_dl = DataLoader(client0_dataset_train,batch_size=1024,num_workers=6,shuffle=True)
    test_dl = DataLoader(client0_dataset_test,batch_size=1024,num_workers=6,shuffle=False)
...
def train(train_dl,test_dl,target_model,attack_model,device,epoch):
    ...
    # optim = torch.optim.AdamW(attack_model.parameters(),lr=0.001,betas=(0.9,0.999),eps=1e08,weight_decay=0.002,amsgrad=False)
...
# def test(test_dataloader,model,device,epoch):
#     t0 = time.time()
...
#                 l = item[0].unsqueeze(1).to(torch.float32).to(device)

#                 h={}
#                 g={}
#                 for key in item[1].keys():
#                     h[key] = item[1][key].to(device)
#                     if 'fc' in key:
#                         g[key] = item[2][key].unsqueeze(1).to(device)
#                     else:
#                         g[key] = item[2][key].to(device)
#                 yhat = item[3].to(device)
#                 y = item[4].to(device)
#                 ll = item[5].to(device)
                
#                 output = model(l,h,g,yhat,y)
#                 loss = criterion(output,ll)
...
#                 test_loss += loss.item()
#                 train_correct_cnt += (output.argmax(dim=1) == ll).sum().item() 
#                 train_ += len(ll)

#     test_loss /= len(test_dataloader)
#     test_acc = (train_correct_cnt / train_) * 100
...
class MkFlDataset(Dataset):
    def __init__(self,dataset_trained,dataset_notrained,samp_idxs,train=True,idx2=[]):
        # print(samp_idxs)
        ...
        # random.shuffle(self.idx1)
        # random.shuffle(self.idx2)
...
if __name__ == "__main__":
    ...
    epoch = 200
    if args.attack_server_only == False:
