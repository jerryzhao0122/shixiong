...
def fl_params():
    ...
    parser.add_argument('--ldp_epsilon',type=float,default=10,help='差分隐私参数epsilon')
    parser.add_argument('--ldp_delta',type=float,default=0.00001,help='差分隐私参数delta')
    ...
    parser.add_argument('--cdp_clip',type=float,default=15,help="cdp的裁切边界")
    parser.add_argument('--cdp_sigma',type=float,default=2,help='sigma value')
    parser.add_argument('--cdp_epsilon',type=float,default=32,help='The epsilon for epsilon-delta privacy')
    parser.add_argument('--cdp_delta',type=float,default=0.0001,help='The defined delta value')
    ...
    parser.add_argument('--rsa_alpha',type=float,default=0.01)
    parser.add_argument('--rsa_l1_lambda',type=float,default=0.002)
    parser.add_argument('--rsa_weight_lambda',type=float,default=0.04)
    # DP-RSA
    parser.add_argument('--dprsa',type=bool,default=False,help='使用DP-RSA')
    parser.add_argument('--dprsa_type',type=str,default='F',help='DP-RSA(F/G)')
    parser.add_argument('--dprsa_f_gamma',type=float,default=0.6,help='DP-RSA(F')
    parser.add_argument('--dprsa_g_epslion',type=float,default=0.2,help='DP-RSA(G)')
    ...
    parser.add_argument('--onud_server_alpha',type=float,default=0.0001,help='服务器聚合的sign前面的参数')
    ...
    # SIGN
    parser.add_argument('--sign',type=bool,default=False)
    parser.add_argument('--sign_v2',type=bool,default=False)
    parser.add_argument('--sign_alpha',type=float,default=0.00001)
    parser.add_argument('--sign_mask',type=bool,default=False)
    parser.add_argument('--sign_mask_p',type=float,default=0.1)
    parser.add_argument('--sign_noise',type=bool,default=False)
    parser.add_argument('--sign_noise_sigma',type=float,default=0.001)
    ...
    parser.add_argument('--attack_num',type=int,default=1,help='恶意客户机的比例')

    #Attack Backdoor Replace
    parser.add_argument('--attack_replace_round',type=int,default=150,help='')
    parser.add_argument('--backdoor_replace_data_num',type=int,default=64,help='数据替换的数量')
    ...
    parser.add_argument('--labelflip_attack_frequency',type=int,default=1,help='每多少轮攻击一次')

    # Attack Gaussian Noise
    parser.add_argument('--gaussian_attack_frequency',type=int,default=1,help='多少轮攻击一次')

    # Attack Weight Flip
    parser.add_argument('--weightflip_attack_frequency',type=int,default=1,help='多少轮攻击一次')

    # Attack Privacy White-Box 
    parser.add_argument('--white_box',type=bool,default=False)

    # Defense Roubust Attack
    parser.add_argument('--robust_agg',type=bool,default=False,help='开启鲁棒性防御')
    parser.add_argument('--robust_agg_type',type=str,default='krum',help='鲁棒性防御类型【krum,trummean,trummedian,mipc】')
    parser.add_argument('--krum_m',type=int,default=5,help='krum算法每次聚合多少个正确的向量')
    parser.add_argument('--trum_k',type=int,default=3,help='trum算法中的k值')

    # Majority Vote
    parser.add_argument('--majority_vote',type=bool,default=False,help='是否使用majority vote方法')
    parser.add_argument('--majority_vote_eta',type=float,default=0.0001,help='majority vote server learning learning')
    parser.add_argument('--majority_vote_lambda',type=float,default=0.01,help='majority vote server weight learning learning')

    # Attack Detector
    parser.add_argument('--ad_dist',type=str,default='mi',help='用于攻击识别器计算距离时候的距离评估方法')
    parser.add_argument('--save_mipc',type=bool,default=False,help='保存mipc的结果')
