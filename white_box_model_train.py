...
class InputFeature:
    ...
    def __init__(self, L, hidden, y, yhat, grad):
        ...
        self.loss = np.array([L]).astype(np.float32)
        self.lable = np.array(y).astype(np.float32)
        self.yhat = np.array(yhat).astype(np.float32)
    ...
    def __init__(self, selected_conv_layer_names, hidden_layer_features_shape, param_gradients_shape, device):
        ...
        for name in self.selected_conv_layer_names:
            if 'conv' in name:
                ...
                self.grad_feature_extractors[name] = Convolutional2DFeatureExtractor(1,2)
                self.grad_feature_extractors[name].to(self.device)
        ...
        for name in self.selected_conv_layer_names:
            if 'conv' in name:
                ...
                # print(dim_in)
                self.hidden_feature_extractors[name] = FullyConnectedFeatureExtractor(dim_in,2)
                self.hidden_feature_extractors[name].to(self.device)

        self.fc_feature_extractor = FullyConnectedFeatureExtractor(21, 2).to(self.device)

        encoder_input_dim = 22375

        self.encoder = FullyConnectedEncoder(encoder_input_dim,2).to(self.device)

    def forward(self,input_feature):
        fc_feature = torch.from_numpy(input_feature.flatten_vector).to(self.device)
        x_list = [self.fc_feature_extractor(fc_feature)]

        for name in self.selected_conv_layer_names:
            tmp = torch.from_numpy(input_feature.hidden[name]).to(self.device)
            print(tmp.shape)
            x_list.append(self.hidden_feature_extractors[name](tmp))
            if 'fc' in name:
                x_list.append(self.grad_feature_extractors[name](torch.from_numpy(input_feature.grad[name]).unsqueeze(0).to(self.device)))
            else:
                x_list.append(self.grad_feature_extractors[name](torch.from_numpy(input_feature.grad[name]).to(self.device)))
        for x in x_list:
            print('kk:',x.shape)
...
class WhiteBox(nn.Module):
    def __init__(self, selected_layer_names, hidden_layer_features_shape, param_gradients_shape, device):
        super().__init__() 
        self.device = device
        self.selected_layer_names = selected_layer_names

        self.grad_feature_extractors = {}
        for name in self.selected_layer_names:
            if 'conv' in name:
                input_shape = param_gradients_shape[name]
                self.grad_feature_extractors[name] = CnnForCNNGrad(input_shape)
                self.grad_feature_extractors[name].to(self.device)
            else:
                input_shape = param_gradients_shape[name]
                self.grad_feature_extractors[name] = CnnForFcnGrad(input_shape)
                self.grad_feature_extractors[name].to(self.device)

        self.hidden_feature_extractors = {}
        for name in self.selected_layer_names:
            if 'conv' in name:
                input_shape = hidden_layer_features_shape[name]    
                self.hidden_feature_extractors[name] = CnnForCnnLayer(input_shape)
                self.hidden_feature_extractors[name].to(self.device)
            else:
                input_shape = hidden_layer_features_shape[name][0]
                self.hidden_feature_extractors[name] = FcnForModel(dim_in = input_shape)
                self.hidden_feature_extractors[name].to(self.device)

        self.yhat_feature_extractor = FcnForModel(10,64).to(self.device)
        self.label_feature_extractor = FcnForModel(10,64).to(self.device)
        self.loss_feature_extractor = FcnForModel(1,64).to(self.device)

        encoder_input_dim = 64*13

        self.encoder = FcnForEncoder(encoder_input_dim,2).to(self.device)
    ...
    def forward(self,input_features):
        ...
        # 梯度和输出
        for name in self.selected_layer_names:
            # print(name,input_features.grad[name].shape)
            if 'fc' in name:
                x_list.append(self.grad_feature_extractors[name](torch.from_numpy(input_features.grad[name]).unsqueeze(0).to(self.device)))
            else:
                x_list.append(self.grad_feature_extractors[name](torch.from_numpy(input_features.grad[name]).to(self.device)))
            x_list.append(self.hidden_feature_extractors[name](torch.from_numpy(input_features.hidden[name]).to(self.device)))
        
        x_list.append(self.yhat_feature_extractor(torch.from_numpy(input_features.yhat).to(self.device)))
        x_list.append(self.label_feature_extractor(torch.from_numpy(input_features.lable).to(self.device)))
        x_list.append(self.loss_feature_extractor(torch.from_numpy(input_features.loss).to(self.device)))

        x = torch.cat(x_list,dim=0).view(-1)
        # x = self.yhat_feature_extractor(torch.from_numpy(input_features.yhat).to(self.device))
        # print(x)
        # print(x.shape)
        x = self.encoder(x)
        # print(x)
...
# class cnn_for_fcn_gradients(nn.Module):
#     def __init__(self,input_shape) :
#         super().__init__()
#         self.dim1 = input_shape[0]
#         self.dim2 = input_shape[1]
...
#         self.conv1 = nn.Conv2d(in_channels=1,out_channels=100,kernel_size=(1,self.dim2),stride=(1,1))
#         self.fc1 = nn.Linear(100*self.dim1,)
...
def get_data_loaders(arg,input_features):

    # N = (args.N - 20000) // 4

    defender_train = input_features[:15000]
    defender_test = input_features[15000:20000]
    reserve_train = input_features[30000:45000]
    reserve_test = input_features[45000:50000]

    train_features = defender_train + reserve_train
    # train_labels = np.ones(30000,dtype=np.int64)
    # train_labels[15000:30000]=0
    train_labels1 = torch.ones(15000).to(torch.int64)
    train_labels2 = torch.zeros(15000).to(torch.int64)
    train_labels = torch.cat([train_labels1,train_labels2],dim=0)

    test_features = defender_test + reserve_test
    # test_labels = np.ones(10000,dtype=np.int64)
    # test_labels[5000:10000]=0
    test_labels1 = torch.ones(5000).to(torch.int64)
    test_labels2 = torch.zeros(5000).to(torch.int64)
    test_labels = torch.cat([test_labels1,test_labels2],dim=0)
    ...
    # test_dataloader = DataLoader(test_dataset,batch_sampler=64,num_workers=10)
...
def train(train_dataloader, model, device, optim, epoch, args):
    ...
    for batch_idx, (X, y) in enumerate(train_dataloader):
        ...
        #     print('requires grad',params.requires_grad)
        optim.step()
...
if __name__ == "__main__":
    ...
    wandb_dir = "/home/featurize/result/wandb_logs"
    ...
    model = WhiteBox(selected_conv_layer_names,hidden_shape,grad_shape,device)
    ...
    # train(train_dataloader, model, device, optim, 1, args)

    # optim = torch.optim.SGD(model.parameters(), lr=0.01)
    # criterion = torch.nn.CrossEntropyLoss()
    # m1 = FcnForModel(10,2)
    # # m2 =FcnForEncoder(64,2)
    # for batch_idx, (X, y) in enumerate(train_dataloader):
    #     x = torch.rand(10)
    #     print('x',x)
    #     x1 = m1(x)
    ...
    white_box_attack_NN_results_dir = "/home/featurize/result/white_box_attack_NN_results"
